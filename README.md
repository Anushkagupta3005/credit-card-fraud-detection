...
**Global feature importance** — which signals the model relies on across all transactions:

![SHAP global feature importance](shap_importance.png)

**Beeswarm** — direction and spread of each feature's effect (colour = feature value):

![SHAP beeswarm](shap_beeswarm.png)

**Local explanation** — one caught fraud, decomposed feature by feature from the model's base rate to its final score:

![SHAP waterfall for a single caught fraud](shap_waterfall.png)
...
```
.
├── credit-card-fraud-detection.ipynb   # end-to-end notebook (EDA -> model -> threshold -> SHAP)
...
├── shap_beeswarm.png                   # SHAP beeswarm (direction & spread)
├── shap_waterfall.png                  # SHAP local explanation (one caught fraud)
├── README.md
└── requirements.txt
```
...
3. Open `credit-card-fraud-detection.ipynb` and run all cells top to bottom. The LightGBM search takes a
